...
async def sync_all_tables():
    """
    Synchronizes the 'registrations' collection with 'event_level_registrations'.
    This script is designed to handle offline entries with older dates correctly.
    """
    client = AsyncIOMotorClient('mongodb+srv://vaividhya2k26:vaividhya1234@cluster0.j2coack.mongodb.net/')
    db = client.vaividhya_db
    
    # 1. Fetch Master Event Mapping
    events_list = await db.events.find({}, {"_id": 0}).to_list(100)
    event_map = {e.get("event_id") or e.get("slug"): e for e in events_list}
    
    # 2. Fetch All Registrations (including those with old dates added recently)
    regs = await db.registrations.find().to_list(5000)
    print(f"Syncing {len(regs)} registrations to event-level tables...")
    
    flattened = []
    for r in regs:
        # User base info - preserving original registration date/time
        user_info = {
        ...
        # Process Events from either 'event_details' or 'selected_events'
    ...
    # 3. Clear and Refresh the Event-Level Table
    await db.event_level_registrations.delete_many({})
    if flattened:
        # Sort by date descending so recently added 'old' entries appear in correct order
        flattened.sort(key=lambda x: (x.get("register_date", ""), x.get("register_time", "")), reverse=True)
        await db.event_level_registrations.insert_many(flattened)
        
    print(f"SUCCESS: Synchronized {len(flattened)} event-level entries.")
